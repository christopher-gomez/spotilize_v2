...
export default class AnimationManager {
  ...
  private static _mainInstance: AnimationManager;
  ...
  constructor(
    ...
    if (renderer === undefined && container === undefined && camera === undefined && AnimationManager._mainInstance !== undefined) {
      return AnimationManager._mainInstance;
    }
    ...
    if (
      ...
      if (!AnimationManager._mainInstance) {
        AnimationManager._mainInstance = this;
      }
    }
  }
  ...
  private updateAnims(dtS: number) {
    if (this._animationDict.animations.length > 0) {
      ...
      let animation: Animation | null = this._animationDict.animations[0];
    ...
    for (const queue in this.animationQueues) {
      ...
      let queueAnim: Animation | null = animationQueue.animations[0];
      let foundNextValid = false;
  ...
  addAnimation(
    animation: Animation,
    animationQueue: string | null = null,
  ...
  getCurAnimInQueue(name: string): Animation | null {
  ...
  dispose(): void {
    ...
    this._animationDict = { cur: 0, animations: [] }; // Assign an empty object instead of null
