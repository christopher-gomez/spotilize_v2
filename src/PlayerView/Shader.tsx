...
export default ({
  ...
  const [scene, setScene] = React.useState<THREE.Scene | Array<THREE.Scene> | null>(
    null
  );
  ...
  const [updateRef, setUpdateRef] = React.useState<
    UpdateFunction | Array<UpdateFunction> | (() => UpdateFunction | UpdateFunction[]) | null // Update the type to allow for null
  >(null);
  const [onLoadedRef, setOnLoadedRef] = React.useState<
    OnLoadFunction | Array<OnLoadFunction> | null // Update the type to allow for null
  >(null);
  ...
  const findCurrentPropertyFromTime = (
    ...
    if (!analysisRef.current) return { current: null, index: -1, object: null };
        ...
        onLoadedRef !== null && (
          <ThreeCanvas
            ...
            onAnimate={async (
              ...
              if (meshRef.current) {
                (meshRef.current.material as THREE.ShaderMaterial).uniforms["time"].value += dt;
              }
              ...
              if (analysisRef.current && playerRef.current) {
                ...
                if (
                  ...
                  if (meshRef.current) {
                    ...
                    (meshRef.current.material as THREE.ShaderMaterial).uniforms["beatConfidence"].value =
                      Tools.lerpFactor(
                        (meshRef.current.material as THREE.ShaderMaterial).uniforms["beatConfidence"]
                          .value,
                        targetConfidence,
                        factor
                      );
