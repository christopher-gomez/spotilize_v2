...
export const SpotifyPlayer = ({
  ...
  useEffect(() => {
    ...
    currentTrackDoesNotHaveAnalysis.current = false;
  }, [currentTrack]);

  const nowPlayingAnalysisRef = useRef(nowPlayingAnalysis);
  const currentTrackDoesNotHaveAnalysis = useRef(false);
  ...
  const handleStateChange = (
    ...
    if (currentTrackRef.current !== trackURI) {
    ...
    if (
      playerRef.current &&
      !currentTrackDoesNotHaveAnalysis.current &&
  ...
  const checkTimeoutRef = useRef<number | undefined>();

  const [stateToken, setStateToken] = useState<string | null>(null);
  const tokenRef = useRef<string | null>(stateToken);
  ...
  const [userPlaylists, setUserPlaylists] =
    useState<SpotifyUserPlaylistsResponse | null>(null);
  ...
  const fetchAnalysis = async (uri, onComplete?: () => void) => {
    try {
      ...
      if (data !== null) {
        ...
        currentTrackDoesNotHaveAnalysis.current = true;
      }
  ...
  const [searchResults, setSearchResults] =
    React.useState<SpotifySearchResponse | null>(null);
